...
(defcustom wtag-artwork-load-preview t
  "non-nil なら `wtag-artwork-load' が Interactive のときプレビューされる."
  :type 'boolean
  :group 'wtag)
...
(defface wtag-track-number-shadow
  '((t :inherit wtag-disk-number-other))
  "wtag-track-number-shadow-face."
  :group 'wtag-faces)
...
(defface wtag-image-size
  '((t :inherit font-lock-type-face :distant-foreground "dark green"))
  ...
  (setq name
        (cond
         ((and (natnump wtag-buffer-name)
               wtag-buffer-name
               (< wtag-buffer-name (string-width name)))
    ...
    (dolist (a index)
      ...
      (insert
       ...
       (let* ((tmp
              ...
              (pos (string-match "/" tmp)))
         (if pos
             (progn 
               (put-text-property 0 pos 'face 'wtag-track-number tmp)
               (put-text-property pos (length tmp) 'face 'wtag-track-number-shadow tmp)
               tmp)
           (put-text-property 0 (length tmp) 'face 'wtag-track-number tmp)
           tmp))
       ;; Time.
       " "
...
(defun wtag-different-cover (event)
  ...
  (let* ((display-buffer-base-action wtag-different-cover-display-action)
    ...
    (with-current-buffer buff
      ...
      (wtag-set :base-name album) ; Trick for `wtag-artwork-write'(`wtag-make-artwork-name').
...
(defun wtag-y-or-n-p (prompt command)
  (or (eq wtag-without-query t)
...
(defvar-local wtag-artwork-select-last-directory nil)

(defun wtag-artwork-select (directory)
  "擬似プレビュー付画像セレクタ.
DIRECTORY からファイル名をひとつ選びプレビューバッファに表示する.
そこで y or n の問い合わせが入る.
`y' でそのファイルが選ばれ `n' で再びファイル名でのセレクトに戻る.
キャンセルは `C-g'."
  (let* ((display-buffer-base-action wtag-different-cover-display-action)
         (file (read-file-name "image: " directory))
         (buff (current-buffer)))
    (and (string-match "\\.\\(jpg\\|jpeg\\|png\\)\\'" file)
         (with-temp-buffer
           ...
           (image-transform-fit-to-window)
           (unwind-protect
               (if (y-or-n-p (apply #'format "[%s %d x %d] OK?" (wtag-image-size))) file)
             (with-current-buffer buff
               (setq-local wtag-artwork-select-last-directory
                           (file-name-directory file)))
             (delete-window))))))

(defun wtag-artwork-load (file-or-object &optional name no-disp no-modified)
  "ファイルまたはオブジェクトをアートワーク・バッファにロードする.
...
またカスタム変数 `wtag-artwork-load-preview' が NON-NIL なら
interactive のときロード前にプレビュー表示され、問い合わせが入る. デフォルト t."
  (interactive
   (let (file)
     (if wtag-artwork-load-preview
         (while (null (setq file
                            (wtag-artwork-select
                             wtag-artwork-select-last-directory))))
